...
""""
    g_algo = GraphAlgo()
    g_algo.load_from_json("data/G_10_80_2")
    g_algo.plot_graph()
    g = nx.DiGraph()
    g.add_nodes_from(g_algo.graph.get_all_v())
    g.add_weighted_edges_from(g_algo.graph.get_all_e())

    start_time = datetime.datetime.now()
    print(nx.shortest_path(source=0, target=9, weight='weight', G=g))
    end_time = datetime.datetime.now()
    delta = end_time - start_time
    print("shortest path: ", delta.microseconds)

    start_time = datetime.datetime.now()
    print(nx.strongly_connected_components(G=g))
    end_time = datetime.datetime.now()
    delta = end_time - start_time
    print("connected_components: ", delta.microseconds)
    """
